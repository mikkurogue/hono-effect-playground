...
import { bulkDeleteRepositoriesSchema, createRepositorySchema, repositoryFilterSchema } from "./schema";

export const repositoriesModule: Effect.Effect<Hono<EvlogVariables>> = Effect.gen(
	function* () {
		const app = yield* createHonoModule();

		yield* app.post(
			"/search",
			zValidator("json", repositoryFilterSchema),
			async (ctx) => {
					...
					getRepositories(filter).pipe(
						...
						Effect.catchTags({
							RepositoryValidationError: (error) => {
								log.error(
									`Repository response validation failed: ${error.message}`,
								);
							...
							RepositoryDatabaseError: (error) => {
								log.error(
									`Database error while listing repositories: ${error.message}`,
								);
								return Effect.succeed({
									ok: false as const,
									status: 500 as const,
				...
				const result = await Effect.runPromise(
					getRepositoryById(id).pipe(
						...
						Effect.catchTags({
							...
							RepositoryDatabaseError: (error) => {
								log.error(
									`Database error while fetching repository: ${error.message}`,
								);
								return Effect.succeed({
									ok: false as const,
									status: 500 as const,
		...
		yield* app.delete(
			"/bulk",
			zValidator("json", bulkDeleteRepositoriesSchema),
			async (ctx) => {
				const log = ctx.get("log");
